...
export async function handler(args: Record<string, unknown>): Promise<string> {
  ...
  switch (action) {
    ...
    case "high_seas": {
      if (!args.id) return "Error: id is required for high_seas action";
...
function formatCompanyList(data: unknown, key: string): string {
  const result = data as Record<string, { total: number; data: Array<Record<string, unknown>> }>;
  const conn = result[key];
  if (!conn) return JSON.stringify(data, null, 2);
  ...
  for (const c of companies) {
    ...
    const claimBy = c.claim_by as Record<string, string> | undefined;
    const claimByGroup = c.claim_by_group as Record<string, { name: string }> | undefined;
    output += `- ${c.name} (ID: ${c.id})\n`;
    output += `  简称: ${c.common_name ?? "-"} | 等级: ${c.rank ?? "-"}\n`;
    output += `  区域: ${region?.name ?? "-"} (${region?.group ?? "-"})\n`;
    output += `  负责人: ${claimBy?.name ?? "-"} | 所属组: ${claimByGroup?.name ?? "-"} | 业务单元: ${claimByGroup?.parent?.name ?? (c as Record<string, unknown>).first_group_name ?? "-"}\n\n`;
  }
  return output;
}
