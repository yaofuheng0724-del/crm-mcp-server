...
export async function handler(args: Record<string, unknown>): Promise<string> {
  ...
  switch (action) {
    ...
    case "detail": {
      if (!args.id) return "Error: id is required for detail action";
    ...
    case "cost": {
      if (!args.id) return "Error: id is required for cost action";
...
function formatProjectList(data: unknown): string {
  const result = data as { listProject?: { total: number; data: Array<Record<string, unknown>> } };
  if (!result.listProject) return JSON.stringify(data, null, 2);
  ...
  for (const p of projects) {
    const company = p.company as Record<string, string> | undefined;
    const claimBy = p.claimBy as Record<string, { name: string }> | undefined;
    const dealAmount = p.deal_amount as Record<string, string> | undefined;
    const actualAmount = p.actual_amount as Record<string, string> | undefined;
    ...
    if (dealAmount?.value) output += `  预估金额: ${dealAmount.value} ${dealAmount.currency ?? ""}`;
    if (actualAmount?.value && actualAmount.value !== "0.00") output += ` | 实际金额: ${actualAmount.value} ${actualAmount.currency ?? ""}`;
    if (dealAmount?.value || actualAmount?.value) output += "\n";
    output += "\n";
  }
  return output;
}
