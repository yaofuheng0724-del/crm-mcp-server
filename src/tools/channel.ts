...
function formatChannelList(data: unknown): string {
  const result = data as { listChannel?: { total: number; data: Array<Record<string, unknown>> } };
  if (!result.listChannel) return JSON.stringify(data, null, 2);

  const { total, data: channels } = result.listChannel;
  let output = `共 ${total} 个伙伴\n\n`;

  for (const ch of channels) {
    const region = ch.region as Record<string, string> | undefined;
    const saleClaimBy = ch.sale_claim_by as Record<string, string> | undefined;
    const saleClaimByGroup = ch.sale_claim_by_group as Record<string, { name: string }> | undefined;
    const chSaleClaimBy = ch.channel_sale_claim_by as Record<string, string> | undefined;
