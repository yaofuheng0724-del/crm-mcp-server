...
// crm_query_project
server.tool("crm_query_project", `查询CRM项目信息。支持以下action：
...
// crm_mutate_project
server.tool("crm_mutate_project", `变更CRM项目数据。支持以下action：
...
// crm_query_company
server.tool("crm_query_company", `查询CRM客户信息。支持以下action：
...
// crm_query_channel
server.tool("crm_query_channel", `查询CRM伙伴信息。支持以下action：
...
server.tool("crm_contract_finance", `操作CRM合同、回款、确认收入和应收计划。按domain分组：
...
// crm_followup
server.tool("crm_followup", `管理CRM跟进记录和评论。支持以下action：
...
// crm_delivery
server.tool("crm_delivery", `操作CRM交付与库存相关功能。按domain分组：
...
// crm_misc
server.tool("crm_misc", `其他CRM业务操作。按domain分组：
